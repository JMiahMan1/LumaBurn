...
2. **Everything is inside a background rect** — LumaBurn automatically removes white/transparent rects that match the document size. If all your design was *inside* such a rect as a clipping group, it may be filtered. Flatten the structure in Inkscape/Illustrator before importing.
