...
def run_raster(device, raster_data: list[str], step_size: int, speed: float, args: argparse.Namespace) -> list:
    results = []
    print(f"[raster] starting {len(raster_data)} rows at {speed} mm/s")
    
    for y, row in enumerate(raster_data):
        if y % 20 == 0:
            print(f"[raster] row {y}/{len(raster_data)}")
        ...
        while x < len(row):
            bit = row[x]
            count = 0
            while x < len(row) and row[x] == bit:
                count += 1
                x += 1
            
            dist_str = encode_distance(count * step_size)
            if bit == "1":
                row_buffer.append("IDS1P\n")
                row_buffer.append(f"IB{dist_str}S1P\n")
                row_buffer.append("IUS1P\n")
            else:
                row_buffer.append(f"IB{dist_str}S1P\n")
            
            # Send in batches of 15 commands to keep the buffer happy
            if len(row_buffer) >= 15:
                device.send_stream("".join(row_buffer))
                row_buffer = []
        
        if row_buffer:
            device.send_stream("".join(row_buffer))
        
        # Return and Step (STABLE)
        back_x = encode_distance(len(row) * step_size)
        step_y = encode_distance(step_size)
        device.send_stream(f"IT{back_x}R{step_y}S1P\n")
...
def run_layer(device, layer: JobLayer, current_steps: tuple[int, int], args: argparse.Namespace) -> tuple[list, tuple[int, int]]:
    ...
    for index, polyline in enumerate(layer.polylines, start=1):
        if args.log_progress:
            print(f"[{layer.name}] polyline {index}/{total} points={len(polyline)}")
...
def main() -> int:
    ...
    parser.add_argument("--raster-json", type=str, default=None, help="Path to JSON file containing bitstrings.")
    parser.add_argument("--raster-speed", type=float, default=150.0, help="Raster speed in mm/s.")
    ...
    parser.add_argument("--polyline-pause-ms", type=float, default=30.0, help="Pause between lit polylines.")
    ...
    try:
        ...
        if args.raster_json:
            import json
            with open(args.raster_json, "r") as f:
                raster_data = json.load(f)
            if args.engrave_power:
                device.set_power(args.engrave_power)
            run_raster(device, raster_data, step_size=4, speed=args.raster_speed, args=args)
            return 0
