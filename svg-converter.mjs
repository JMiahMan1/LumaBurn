import { combineTransforms, parseTransform } from "./src/core/math.mjs";
...
function convertElement(element, defsMap, options) {
  ...
  switch (tagName) {
    case "g":
      return convertGroup(element, defsMap, options, base);
    case "path":
      return convertPath(element, base);
    case "rect":
      return convertRect(element, base);
    case "circle":
      return convertCircle(element, base);
    case "ellipse":
      return convertEllipse(element, base);
    case "line":
      return convertLine(element, base);
    case "polyline":
      return convertPolyline(element, base);
    case "polygon":
      return convertPolygon(element, base);
    case "use":
      return convertUse(element, defsMap, options, base);
    case "text":
      return convertText(element, base);
    case "image":
      return convertImage(element, base);
    default:
      base.type = "group";
      base.name = tagName;
      base.children = [];
...
export function nodeTreeToSvgString(node, parentTransform = null, parentStyles = {}) {
  ...
  if (Object.keys(mergedStyle).length > 0) {
    attrs.style = Object.entries(mergedStyle)
      .map(([k, v]) => `${k}:${v}`)
      .join(";");
  }
