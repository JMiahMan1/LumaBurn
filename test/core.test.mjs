...
test('buildGcodeFromPolylines exhaustive branches', () => {
  ...
  const layer = { id: 'L1', enabled: true, name: 'Cut', power: 100, feed: 1000, passes: 2, airAssist: true, mode: 'score' };
  const op = { operationLayer: layer, polylines: [[{x:0,y:0}, {x:10,y:10}]] };
...
test('estimateJobFromPolylines branch coverage', () => {
  ...
  const op2 = { operationLayer: { ...layer, enabled: false }, polylines: [[{x:0,y:0}, {x:10,y:10}]] };
...
test('applyLineStyleToPolylines branch coverage', () => {
  const polyline = [{x:0,y:0}, {x:20,y:0}];
...
test('buildGcodeFromPolylines feature branches', () => {
  ...
  const layer = { id: 'L1', name: 'Cut', enabled: true, mode: 'line', feed: 1000, power: 100, passes: 1, airAssist: true, constantPower: true };
  const ops = [{ operationLayer: layer, polylines: [[{x:0, y:0}, {x:10, y:10}]] }];
...
test('surgical gcode branch coverage', async () => {
  ...
  const okTags = ['ok','start','started','queued','running','processing','uploaded','done','success','[SD-RUN]','[ESP220]','file opened','stream started','[MSG:'];
  okTags.forEach(t => assert.ok(inspectDeviceResponse(t).ok, `Should match ${t}`));
  const failTags = ['error','fail','failed','invalid','unknown','busy','denied','missing','timeout','forbidden'];
  ...
  assert.ok(buildRunFileCommands('/sd/test.gc', { controllerFlavor: 'grbl-embedded' }).some(c => c.includes('[ESP220]')));
  assert.equal(canUseControllerFileRun({ storageMode: 'direct', uploadPath: '' }), false);
  ...
  await assert.rejects(executeStopSequence({ sendCommand: () => { throw new Error('!!'); } }), /!!/);
