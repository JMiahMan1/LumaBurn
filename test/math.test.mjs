...
test('formatCompact neatly rounds floats to precision strings', () => {
  ...
  assert.equal(formatCompact(1.5000), '1.5');
...
test('objectWorldBounds branch coverage for mixed groups', () => {
  const rect = { type: 'rect', sourceBounds: { minX:0, minY:0, width:10, height:10 }, matrix: { a:1, b:0, c:0, d:1, e:0, f:0 } };
  const path = { type: 'path', sourceBounds: { minX:10, minY:10, width:10, height:10 }, matrix: { a:1, b:0, c:0, d:1, e:0, f:0 } };
  const group = {
    type: 'group',
...
test('objectWorldBounds handles specialized types', () => {
  assert.strictEqual(objectWorldBounds({ type: 'image', sourceBounds: {minX:0,minY:0,width:5,height:5} }).width, 5);
  assert.strictEqual(objectWorldBounds({ type: 'text', sourceBounds: {minX:0,minY:0,width:10,height:2} }).width, 10);
...
test('composeTransform: scaleX/scaleY and legacy scale produce correct output', () => {
  ...
  assert.ok(composeTransform(withNewFields).includes('scale(2 3)'),
    'scaleX/scaleY must appear independently in transform');

  // Legacy fallback: no scaleX/scaleY -> uses scale for both -> scale(2 2)
  assert.ok(composeTransform(withLegacyField).includes('scale(2 2)'),
