...
async function startMockLaser() {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => { if (!resolved) reject(new Error("Mock laser timeout")); }, 3000);
  });
}
...
async function startTestServer() {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => { if (!resolved) reject(new Error("Server start timed out")); }, 5000);
  });
}
...
test("LumaBurn E2E: Complete Workflow Audit", async (t) => {
  ...
  page.on("console", (msg) => {
    const text = msg.text();
    if (msg.type() === "error") {
        if (!text.includes("favicon") && !text.includes("/network-info") && !text.includes("/list-ports") && !text.includes("404")) {
  ...
  try {
    await t.test("Initial Load & UI Stability", async () => {
      ...
      assert.deepEqual(elementAudit.failures, [], `The following elements failed to bind: ${elementAudit.failures?.join(", ")}`);
    ...
    assert.ok(gcode.includes("M3") || gcode.includes("M4") || gcode.includes("M5") || gcode.includes("S"), "G-code should contain laser power commands");
  });
    ...
    await t.test("Ray 5 Interaction Audit (Mock Hardware)", async () => {
      ...
      assert.ok(browsePath === "/" || browsePath === "/sd/", `Ray 5 should use a valid storage path, got: ${browsePath}`);
      
      const activity = await page.evaluate(() => window.LumaState.device.activityLog.map(a => a.message).join(" "));
      assert.ok(activity.includes("Loading controller files") || activity.includes("Listing files"), `Activity log should show progress, got: ${activity}`);
    });
    ...
    await t.test("Ray 5 Advanced Command & Upload Audit", async () => {
      ...
      const audit = await page.evaluate(async ({ mUrl }) => {
      ...
      const postUploadAudit = await page.evaluate(async ({ mUrl }) => {
        const res = await fetch(`${mUrl}/test/audit`);
        return await res.json();
      }, { mUrl: mockUrl });

      assert.ok(postUploadAudit.uploadCount >= 1, "Mock laser should have registered at least one upload");
      assert.ok(postUploadAudit.files.some(f => f.name.includes("upload")), "Uploaded file should appear in mock filesystem");
    });
    ...
    await t.test("Cross-Hardware Compatibility Audit (MKS & FluidNC)", async () => {
      ...
      for (const scenario of hardwareScenarios) {
        ...
        await page.evaluate(async ({ mUrl, pType }) => {
        ...
        await page.waitForFunction(() => {
          return window.LumaState.device.lastFileSummary?.includes("file") || 
