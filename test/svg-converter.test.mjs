...
test("convertSvgToNodes: parses shapes (circle, ellipse, line, polyline, polygon)", () => {
  ...
  assert.equal(result.nodes[0].type, "circle");
  assert.equal(result.nodes[0].r, 5);
  assert.equal(result.nodes[1].type, "ellipse");
  assert.equal(result.nodes[1].rx, 10);
  assert.equal(result.nodes[2].type, "line");
  assert.equal(result.nodes[2].x2, 3);
  assert.equal(result.nodes[3].type, "polyline");
  assert.deepEqual(result.nodes[3].points, [
    { x: 0, y: 0 },
    { x: 10, y: 10 },
    { x: 20, y: 0 },
  ]);
  assert.equal(result.nodes[4].type, "polygon");
  assert.deepEqual(result.nodes[4].points, [
    { x: 5, y: 5 },
    { x: 15, y: 5 },
    { x: 10, y: 15 },
  ]);
...
test("nodeTreeToSvgString: formats node tree to svg string correctly for all types", () => {
  const root = {
    ...
    style: { opacity: 0.5 },
    class: "layer",
    transform: { matrix: { a: 1, b: 0, c: 0, d: 1, e: 10, f: 10 } },
    children: [
...
test("nodeTreeToSvgString: handles unknown types gracefully", () => {
  const tree = {
    ...
    attributes: { x: 0 },
    style: {},
    children: [{ tagName: "circle", type: "circle", cx: 10, cy: 10, r: 5, attributes: {}, style: {} }],
  };
...
function assertNodeContract(node, label) {
  ...
  if (node.type === "rect") {
    if (node.width !== undefined) {
      assert.equal(typeof node.width, "number", `${label}: rect.width must be a number`);
    }
    if (node.height !== undefined) {
      assert.equal(typeof node.height, "number", `${label}: rect.height must be a number`);
    }
    if (node.x !== undefined) {
      assert.equal(typeof node.x, "number", `${label}: rect.x must be a number`);
    }
    if (node.y !== undefined) {
      assert.equal(typeof node.y, "number", `${label}: rect.y must be a number`);
    }
  }
  if (node.type === "circle") {
    if (node.r !== undefined) {
      assert.equal(typeof node.r, "number", `${label}: circle.r must be a number`);
    }
    if (node.cx !== undefined) {
      assert.equal(typeof node.cx, "number", `${label}: circle.cx must be a number`);
    }
    if (node.cy !== undefined) {
      assert.equal(typeof node.cy, "number", `${label}: circle.cy must be a number`);
    }
  }
